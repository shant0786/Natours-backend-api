...
const createSendToken = (user, statusCode, res) => {
  ...
  if (process.env.NODE_ENV === 'producetion') cookieOptions.secure = true;
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password? Submit a PATCH request with your new password and passwordConfirm to:${resetURL}.\nIf you didn't foget your password, please ignor this email!`;
  try {
    await sendEmail({
      email: user.email,
    ...
    await user.save({ validateBeforeSave: false });
    return next(
      new AppError('There was a error sending the email. Try again later')
    );
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Check is Posted current paassowrd
  ...
  await user.save();
  // User.findByIdAndUpdate will NOT Work as intended bcoz this will trigger pre(middleware) and validation
